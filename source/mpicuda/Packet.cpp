...
bool tPacket::Extract(vector<BYTE>& raBuffer)
{
...
void tPacket::AppendByte(vector<BYTE>& raBuffer, BYTE btValue)
{
	raBuffer.push_back(btValue);
}

void tPacket::AppendWord(vector<BYTE>& raBuffer, WORD wValue)
{
	AppendByte( raBuffer, LOBYTE(wValue) );
	AppendByte( raBuffer, HIBYTE(wValue) );
}

void tPacket::AppendDword(vector<BYTE>& raBuffer, DWORD dwValue)
{
...
BYTE* tPacket::pop_front(vector<BYTE>& raBuffer, DWORD dwSize)
{
